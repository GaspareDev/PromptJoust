...
class MatchEngine:
    ...
    def __init__(
        ...
        self.hero_prompt = sanitize_tactical_prompt(hero_prompt)
        self.boss = boss
        ...
        self.boss_state = FighterState.from_stats(boss.name, boss.stats)
    ...
    def _execute_round(self, round_num: int) -> RoundLog:
        """
        Executes a single combat round transition:
        1. Snapshots pre-round fighter states.
        2. Builds sandboxed XML prompt for the referee LLM.
        3. Obtains TurnDecision from the configured LLM Provider.
        4. Aligns mechanical actions with player directives to prevent LLM drift.
        5. Computes combat damage, stamina deltas, and status effects.
        6. Updates HP, stamina (+ recovery), and status counters.
        7. Returns complete RoundLog record.
        """
        hero_pre = copy.deepcopy(self.hero_state)
        boss_pre = copy.deepcopy(self.boss_state)

        # 1. Build sandboxed referee prompt
        ...
        # 2. Query LLM Provider for Turn Decision
        turn_decision = self._get_turn_decision(user_prompt, round_num)

        # 3. Intent Alignment Check: Guarantee exact adherence to player tactical directives
        ...
        # 4. Deterministic State Resolution via Rules Engine
        ...
        # 5. Apply Damage
        self.hero_state.current_hp = max(0, self.hero_state.current_hp - boss_res.damage_dealt)
        self.boss_state.current_hp = max(0, self.boss_state.current_hp - hero_res.damage_dealt)

        # 6. Apply Stamina Changes & Per-round Recovery
        ...
        self.boss_state.current_sta = max(
            0, min(self.boss_state.max_sta, self.boss_state.current_sta - boss_res.stamina_spent + boss_res.stamina_recovered + STAMINA_RECOVERY_PER_ROUND)
        )
    ...
    def _get_turn_decision(self, user_prompt: str, round_num: int) -> TurnDecision:
        try:
            ...
            # Fallback to safe default turn decision if provider throws error
            return TurnDecision(
                round_number=round_num,
                hero_intent=HeroIntent(
                    action=ActionType.ATTACK,
                    banter="Forward!",
                    tactical_reasoning=f"Standard strike (arbitration error: {err_msg})"[:140],
                ),
                boss_intent=BossIntent(
                    action=ActionType.ATTACK,
                    banter="Annihilate!",
                    tactical_reasoning="Defaulting to standard attack.",
                ),
